...
async function main() {
  ...
  const response = await fetch(endpoint, {
    headers: { "user-agent": "TrendPulse-Stage03/0.3 (+https://github.com/kavie-cmyk/trend-pulse)" },
    signal: AbortSignal.timeout(30000)
  });

  if (!response.ok) throw new Error(`GDELT request failed: HTTP ${response.status}`);
