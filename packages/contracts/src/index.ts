...
export type SourceComplianceStatus = "verified" | "terms-review-required" | "approval-required" | "restricted" | "unknown";
export type SourceResearchOrigin = "registry" | "web-research" | "user-supplied" | "system-discovery";
export type SourceResearchRole = SourcePlanRole | "exclude";
export type SourceActivationDecision =
  | "activate-now"
  | "activate-when-credentialed"
  | "build-connector"
  | "manual-assisted"
  | "defer-runtime"
  | "paid-later"
...
export interface SourceRegistryEntry {
  ...
  creatorVisibilityScore: number;
  communityVisibilityScore: number;
  ...
  researchOrigin: SourceResearchOrigin;
}

export interface SourceEvaluationFactor {
  key: string;
  label: string;
  score: number;
  note: string;
...
export interface SourceWorkspaceEvaluation {
  sourceId: string;
  workspaceId: string;
  intelligenceFitScore: number;
  operationalFeasibilityScore: number;
  fit: SourceFitLevel;
  role: SourceResearchRole;
  activationDecision: SourceActivationDecision;
  factors: SourceEvaluationFactor[];
  rationale: string[];
  evidenceRefs: string[];
  evaluatedAt: string;
}

export interface SourceResearchGap {
  id: string;
  label: string;
  description: string;
...
export interface SourceResearchResult {
  ...
  evaluatedSources: SourceWorkspaceEvaluation[];
  newCandidateSourceIds: string[];
  gaps: SourceResearchGap[];
